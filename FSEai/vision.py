...
def train_mnist(model, trainset):
    ...
    for epoch in range(num_epochs):
        ...
        print("Epoch {}, Training Accuracy: {} ".format(epoch, (correct / total) * 100))
...
def test_mnist(model, testset):
    ...
    print("Test Accuracy of the model on the test dataset: {}".format((correctly_classified / total_classified) * 100))
...
def visualise_sampledata(samples, dataset):
    ...
    for i in range(0, 6):
        ...
        a[i].set_title("Target = {}".format(target.item()))
        fig.tight_layout()
...
def visualise_test(samples, dataset, model):
    ...
    for i in range(0, 6):
        ...
        _, predicted = torch.max(outputs.data, 1)
        a[i].imshow(data, cmap='gray')
        a[i].set_title("Target = {}, Prediction = {}".format(target.item(), predicted.item()))
        fig.tight_layout()
